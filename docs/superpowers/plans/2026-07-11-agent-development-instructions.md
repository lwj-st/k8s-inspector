...
> 2026-07-12 纠偏说明：
>
> 本文档中的“保存巡检对象、导入导出巡检对象”曾被放入名称空间巡检主流程，现已确认不符合自动巡检产品目标。
> 后续开发必须优先参考 `docs/superpowers/plans/2026-07-12-auto-inspection-product-realignment.md`。
> 名称空间巡检的主路径应调整为：自动发现名称空间 -> 多选或一键巡检全部 -> 异常优先展示 -> 查看 Pod 证据与日志命中。
> 不要再继续强化“手动输入 namespace + 保存巡检对象 + 导入导出巡检对象”的主流程。
